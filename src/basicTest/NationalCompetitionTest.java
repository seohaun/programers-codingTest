 ...
 */
public class NationalCompetitionTest {
    ...
    public static int solution(int[] rank, boolean[] attendance) {
        int answer = 0;
        TreeMap<Integer, Integer> sortNumber = new TreeMap<>();
        for (int i = 0; i < attendance.length; i++) {
            sortNumber.put(rank[i], i);
        }
        int count = 0;
        for (Map.Entry<Integer, Integer> entry : sortNumber.entrySet()) {
            if (!attendance[entry.getValue()]) continue;
            if (count < 3) {
                int value = entry.getValue();
                answer += count != 2 ? Math.pow(10, 4 - count * 2) * value : value;
                count++;
            } else {
                break;
            }
